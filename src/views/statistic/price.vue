<template>
  <div>
    <div class="event-list">
      ...
      />
      <div class="title">
        <div>PREDICTION POOL WHITE + BLACK PRICE</div>
      </div>
      <b-tabs v-if="hashLoad && renderComponent" class="col mt-4 date">
        ...
        >
          ...
          <PlatformStatisticCard
            ...
            :tab-name="item.name"
            :collateralAddress="
              contractsAddressesBySuitArray[suitAddress].collateralToken
            "
          />
          <PlatformStatisticCard
...
import PlatformStatisticCard from "@/components/cards/PlatformStatisticCard";
...
export default {
  ...
  computed: {
    ...
    platformId() {
      return this.$store.getters["switch/switchPlatform/getPlatformId"];
    },
    hashesComputed() {
  ...
  mounted() {
    if (this.isSuitesVisibles === null && !this.isCustomContractsEnabled) {
      this.$router.push("/platforms/suitsPlatforms");
      return;
    }
    this.getHashesForEvent();
  },
  watch: {
    async PlatformId() {
      await this.getHashesForEvent();
      this.calc = false;
  ...
  methods: {
    ...
    async getHashesForEvent() {
      ...
      const objMCustom = { name: "Custom", hashes: [] };
      for (
        let i = 0;
        ...
        const date =
          this.finalResultEvents[suitAddress][eventAddress][i].timeStamp * 1000;
